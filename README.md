...
它不是窗口管理器，也不会修改目标应用本身。单次 `set/apply` 通常只作用于当前已打开的窗口；如需让新打开或重建的窗口继续保持透明度，可保存规则并运行 `watch`。
...
| `rules[].process` | string | 是 | 进程名，大小写不敏感 |
| `rules[].class` | string | 否 | 窗口类名，大小写不敏感，精确匹配 |
| `rules[].title_contains` | string | 否 | 标题子串，大小写不敏感 |
| `rules[].opacity` | integer | 是 | 不透明度百分比，范围 `20` 到 `100` |
| `rules[].enabled` | boolean | 否 | 是否启用规则，默认 `true` |
...
保存或更新一条规则。相同 `process + class + title_contains` 的规则已存在时只更新透明度和启用状态，不重复追加。
...
启动后立即应用一次配置，然后按 `--interval` 轮询可见窗口并持续应用匹配规则。按 `Ctrl+C` 退出。`watch` 不会安装后台服务、不写注册表、不自动开机启动。
...
- 只作用于当前能被后端枚举到的可见窗口。
- 单次 `set/apply` 不是持久化配置；应用重启、窗口重建或桌面环境重载后可能失效。需要持续保持时运行 `wtrans watch`。
